...
**Note:** For an example of how this library can be used in a backend, check out [cf-signify-java-example](https://github.com/veridian-id/cf-signify-java-example). This repository provides a practical implementation reference for those interested in using `cf-signify-java` in real-world applications.
...
It is possible to change the keria image by using environment variables. For example, to use gleif/keria:0.4.1-dev1, do:

```bash
export KERIA_IMAGE_TAG=0.4.1-dev1
docker compose pull
docker compose up --wait
```
